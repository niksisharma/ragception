...
class VectorStore:
    ...
    def _get_api_key(self) -> Optional[str]:
        ...
        # Try environment variable first (safest)
    ...
    def set_reranker(self, reranker):
        """Set the reranker for improved search results"""
        self.reranker = reranker
        logger.info("Reranker attached to vector store")
    ...
    def semantic_search(
        ...
        """
        Search for similar papers using embeddings with optional reranking
        
        Args:
            query: Search query
            n_results: Number of results to return
            use_reranker: Whether to use LLM reranking
            user_interests: User's interests for personalized reranking
        """
        ...
        candidates_needed = n_results * 4 if (use_reranker and self.reranker) else n_results
            ...
            if result['paper_id'] not in seen_papers:
                ...
                if paper:
                    unique_results.append({
                        ...
                        'relevant_chunk': result['chunk_text'][:300] + '...'
                    })
                    seen_papers.add(result['paper_id'])
                    
                    if len(unique_results) >= candidates_needed:
                        break
        
        # Apply reranking if available
        if use_reranker and self.reranker and len(unique_results) > n_results:
            logger.info(f"Reranking {len(unique_results)} candidates to get top {n_results}")
    ...
    def semantic_search_with_explanation(
        self,
        query: str,
        n_results: int = 5
    ) -> tuple[List[Dict], str]:
        """Search with reranking explanation"""
        
        # Get candidates
        query_embedding = np.array(self.create_embedding(query))
        
        self.db.cursor.execute("SELECT paper_id, chunk_index, chunk_text, embedding FROM embeddings")
        ...
        if self.reranker and hasattr(self.reranker, 'rerank_with_explanation'):
            reranked, explanation = self.reranker.rerank_with_explanation(
                query, unique_results, n_results
            )
            return reranked, explanation
        else:
            return unique_results[:n_results], "Ranked by embedding similarity"
    ...
    def get_embedding_stats(self) -> Dict:
        ...
        stats['reranker_enabled'] = self.reranker is not None
